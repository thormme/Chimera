...
bool ComputeShadow(float4 shadowMapPosition, bool hiRes)
{
	float shadowDepth;
	...
	if (hiRes)
	{
		shadowDepth = SAMPLE_TEXTURE(HiResShadowMap, shadowTexCoord).r;
	}
	else
	{
		shadowDepth = SAMPLE_TEXTURE(ShadowMap, shadowTexCoord).r;
	}
